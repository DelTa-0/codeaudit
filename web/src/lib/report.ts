...
${row(["Hardcoded secrets", String(s.counts.secrets ?? 0)])}
${row(["Agent config risks", String(s.counts.agentConfig ?? 0)])}
...
${dependencies
  ...
  // Secrets are persisted as code_findings rows (confidence 1.0), so an
  // unfiltered dump here would render them under "Dead-code findings" with
  // the credential provider standing in for a symbol name — misleading, not
  // a value leak (report.ts:11/api.ts already never expose `fingerprint`),
  // but still wrong. Split them into their own section, matching
  // ScanDetail.tsx's Secrets card.
  ...
  const secretsSection = secretFindings.length
    ? `<h2>Hardcoded secrets (${secretFindings.length})</h2>
<table class="grid"><thead>${row(["Provider", "Location", "Guidance"], "th")}</thead><tbody>
${secretFindings
  .map((f) =>
    row([
      esc(f.detail && "provider" in f.detail ? f.detail.provider : "Unknown provider"),
      esc(`${f.file_path}${f.line_start ? `:${f.line_start}` : ""}`),
      f.detail && "removedFromHead" in f.detail && f.detail.removedFromHead
  ...
  // Agent-config findings never carry raw repo text — `detail.evidence` is
  // already sanitized at the source (see packages/engine/src/agentConfig.ts's
  // redactSnippet), so it's safe to render directly, same as everywhere else
  // this finding type appears (CLI, dashboard, MCP tool response).
  const agentConfigSection = agentConfigFindings.length
    ? `<h2>Agent config risks (${agentConfigFindings.length})</h2>
<p style="font-size:9pt;color:#555">Files an AI coding agent trusts as instructions or configuration. Advisory only — does not affect the score.</p>
<table class="grid"><thead>${row(["Rule", "Severity", "Location", "Guidance"], "th")}</thead><tbody>
${agentConfigFindings
  .map((f) => {
    const detail = f.detail && "rule" in f.detail ? f.detail : null;
    return row([
      esc(detail?.rule ?? f.finding_type),
      esc(detail?.severity ?? "—"),
      esc(`${f.file_path}${f.line_start ? `:${f.line_start}` : ""}`),
      esc(f.llm_reasoning ?? ""),
    ]);
  ...
  const findingsSection = deadCodeFindings.length
    ? `<h2>Dead-code findings (${deadCodeFindings.length})</h2>
